...
parser.add_argument('--print_per',type=int,default=1)
...
train_loader_tgt = utils.data_load(os.path.join('data', args.tgt_data), 'train', tgt_transform, args.batch_size, shuffle=True, drop_last=True)
train_loader_edge = utils.data_load(os.path.join('data', args.tgt_data), 'pair', tgt_transform, args.batch_size, shuffle=True, drop_last=True)
...
if args.latest_generator_model == '':
    ...
    for epoch in range(args.pre_train_epoch):
        ...
        Recon_losses = []
        for index, (x, _) in enumerate(train_loader_src):
            x = x.to(device)
            ...
            if(index % args.print_per == 0):
                count = count+1
                image = torch.cat([x,G_], dim=2)
                image = (image + 1)/2
                printText = '[%d/%d](%d/7141) , Recon loss: %.3f' % ((epoch + 1),args.pre_train_epoch,(index+1)*8,torch.mean(torch.FloatTensor(Recon_losses)))
                temp_loss = (torch.mean(torch.FloatTensor(Recon_losses))).item()
                print(printText)
                Visualization(recon_loss=temp_loss,text=printText,image=image,Type=0,Count=count)

        per_epoch_time = time.time() - epoch_start_time
        pre_train_hist['per_epoch_time'].append(per_epoch_time)
        printText = '[%d/%d] - time: %.2f, Recon loss: %.3f' % ((epoch + 1), args.pre_train_epoch, per_epoch_time, torch.mean(torch.FloatTensor(Recon_losses)))
        print(printText)
...
for epoch in range(args.train_epoch):
    ...
    Gen_losses = []
    Con_losses = []
    for (x, _), (y, _), (e, _) in zip(train_loader_src, train_loader_tgt, train_loader_edge):
        index = index + 1
        ...
        if(index % args.print_per == 0):
            ...
            image = torch.cat([x, G_], dim=2)
            image = (image + 1) / 2
            gen_loss = (torch.mean(torch.FloatTensor(Gen_losses))).item()
            con_loss = (torch.mean(torch.FloatTensor(Con_losses))).item()
